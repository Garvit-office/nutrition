// src/features/nutrition/data/repositories/nutrition-repository-impl.ts
...
import {
  ...
  NutritionRemoteDataSource,
} from '../datasources/nutrition-remote-datasource';
import {
  nutritionRemoteDataSource,
  NutritionRemoteDataSource,
} from '../datasources/nutrition-remote-datasource';
...
export class NutritionRepositoryImpl implements NutritionRepository {
  constructor(
    private readonly local: NutritionRemoteDataSource = nutritionRemoteDataSource,
  ...
  async getDailySummary(date: string): Promise<Result<DailyNutritionSummary, AppFailure>> {
    return fromPromise(
      this.local.getLogsForDate(date).then((items) => summarizeDay(date, items)),
      (error) => new CacheFailure('Unable to load nutrition logs from local storage.', error),
    );
  }
  ...
  async logFood(input: LogFoodInput): Promise<Result<FoodItem, AppFailure>> {
    ...
    const localResult = await fromPromise(
      ...
      (error) => new CacheFailure('Failed to save food log locally.', error),
    );
    ...
    if (online) {
      const syncResult = await fromPromise(
        ...
        (error) => new ServerFailure('Failed to sync food log to the server.', error),
      );
      if (syncResult.ok) {
  ...
  async deleteFoodLog(id: string): Promise<Result<void, AppFailure>> {
    const localResult = await fromPromise(
      ...
      (error) => new CacheFailure('Failed to delete the local food log.', error),
    );
    ...
    if (online) {
      await fromPromise(
        ...
        (error) => new ServerFailure('Failed to delete food log on the server.', error),
      );
    }
    return ok(undefined);
  }
  ...
  async searchFoodCatalog(query: string): Promise<Result<readonly FoodItem[], AppFailure>> {
    ...
    if (!online) {
      return fromPromise(
        ...
        (error) => new NetworkFailure('Offline: showing cached results only.', error),
      );
    }

    const remoteResult = await fromPromise(
      this.remote.searchCatalog(query),
      (error) => new ServerFailure('Failed to search the food catalog.', error),
    );
    ...
    return fromPromise(
      this.local.searchCache(query),
      (error) => new CacheFailure('Failed to search cached food logs.', error),
    );
  }
  ...
  async syncPendingLogs(): Promise<Result<number, AppFailure>> {
    const online = await isOnline();
    if (!online) {
      return err(new NetworkFailure('Cannot sync while offline.'));
    }
    ...
    for (const item of pending) {
      const pushResult = await fromPromise(
        ...
        (error) => new ServerFailure(`Failed to sync log ${item.id}.`, error),
      );
